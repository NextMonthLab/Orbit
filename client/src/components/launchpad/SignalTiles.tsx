  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4" data-testid="signal-tiles">
      {tiles.map((tile) => (
        <div
          key={tile.label}
          className="bg-white/5 border border-white/10 rounded-lg p-3 md:p-4 transition-colors hover:border-white/20"
          data-testid={`tile-${tile.label.toLowerCase().replace(" ", "-")}`}
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-white/60 text-xs md:text-sm">{tile.label}</span>
            <div className={`${tile.bgColor} p-1 md:p-1.5 rounded`}>
              <tile.icon className={`w-3 h-3 md:w-4 md:h-4 ${tile.color}`} />
            </div>
          </div>
          <div className="text-xl md:text-2xl font-bold text-white">{tile.value.toLocaleString()}</div>
        </div>
      ))}
    </div>
  );
