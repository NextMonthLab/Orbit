...
-   **Story Fidelity Modes (Structural Ingest)**: Content uploads now go through a Structural Ingest step that detects content type (script vs article). Scripts trigger Script-Exact Mode which parses scene headings (INT./EXT.), characters, dialogue blocks, and action into an internal scene map. Cards are generated from scenes, not themes, preserving narrative order and character intent. Interpretive Mode (default for non-scripts) uses the original summarization behavior.
