...
-   **Industry Orbit Seed API**: Complete data import system for Industry Orbits with 11 entity tables (entities, products, specs, reviews, assets, communities, tiles, pulse sources/snapshots/events, alignments). POST `/api/industry-orbits/:slug/seed` imports JSON seed packs with Zod validation. GET `/api/industry-orbits/:slug/definition` returns full orbit data with stats. Enforces Orbit Type Doctrine: seed API returns 403 for non-industry orbits.
